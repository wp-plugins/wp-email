...
function email_js() {
	...
	echo "\t".'var email_text_max_friend_names_allowed = \''.js_escape(sprintf(__ngettext('- Maximum %s Friend Name allowed', '- Maximum %s Friend Names allowed', $email_max, 'wp-email'), $email_max)).'\';'."\n";
	...
	echo "\t".'var email_text_max_friend_emails_allowed = \''.js_escape(sprintf(__ngettext('- Maximum %s Friend Email allowed', '- Maximum %s Friend Emails allowed', $email_max, 'wp-email'), $email_max)).'\';'."\n";
	...
	echo '/* ]]> */'."\n";
	echo '</script>'."\n";
	wp_register_script('wp-email', plugins_url('wp-email/email-js-packed.js'), false, '2.31');
	...
	if(@file_exists(TEMPLATEPATH.'/email-css.css')) {
		...
		echo '<link rel="stylesheet" href="'.plugins_url('wp-email/email-css.css').'" type="text/css" media="screen" />'."\n";
	}	
...
function email_multiple($echo = true) {
	...
	if($email_multiple > 1) {
		$output = '<br /><em>'.sprintf(__ngettext('Separate multiple entries with a comma. Maximum %s entry.', 'Separate multiple entries with a comma. Maximum %s entries.', $email_multiple, 'wp-email'), $email_multiple).'</em>';
		if($echo) {
			echo $outut;
		} else {
			return $output;
...
function process_email_form() {
	...
	if(!empty($_POST['wp-email'])) {
		...
		if(sizeof($friends) > $multiple_max) {
			$error .= '<br /><strong>&raquo;</strong> '.sprintf(__ngettext('Maximum %s Friend allowed', 'Maximum %s Friend(s) allowed', $multiple_max, 'wp-email'), $multiple_max);
		}
		if(intval($email_fields['friendname']) == 1) {
...
function email_page_admin_most_stats($content) {
	...
	if($stats_display['emailed_most'] == 1) {
		$content .= '<input type="checkbox" name="stats_display[]" id="wpstats_emailed_most" value="emailed_most" checked="checked" />&nbsp;&nbsp;<label for="wpstats_emailed_most">'.sprintf(__ngettext('%s Most Emailed Post', '%s Most Emailed Posts', $stats_mostlimit, 'wp-email'), $stats_mostlimit).'</label><br />'."\n";
	} else {
		$content .= '<input type="checkbox" name="stats_display[]" id="wpstats_emailed_most" value="emailed_most" />&nbsp;&nbsp;<label for="wpstats_emailed_most">'.sprintf(__ngettext('%s Most Emailed Post', '%s Most Emailed Posts', $stats_mostlimit, 'wp-email'), $stats_mostlimit).'</label><br />'."\n";
	}
	return $content;
}
...
function email_page_most_stats($content) {
	...
	$stats_mostlimit = intval(get_option('stats_mostlimit'));
	if($stats_display['emailed_most'] == 1) {
		$content .= '<p><strong>'.sprintf(__ngettext('%s Most Emailed Post', '%s Most Emailed Posts', $stats_mostlimit, 'wp-email'), $stats_mostlimit).'</strong></p>'."\n";
		$content .= '<ul>'."\n";
